    ...
    const fetchTabData = async () => {
      ...
      try {
        if (activeTab === "clients" && !clientData) {
          ...
          const res = await fetch("/api/reports/holdings");
          const data = await res.json();
          setHoldingData(data);
            ...
            <>
              <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                ...
                <ReportCard
                  ...
                  value={formatCurrency(overview.kpis.totalPending)}
                ...
                <ReportCard
                  ...
                  subtitle={`${overview.kpis.activeBookings} active bookings`}
                  icon={Percent}
                  accentColor="indigo"
                />
                <ReportCard
                  title="Pending Tasks"
                  value={overview.kpis.pendingTasks}
              ...
              <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                <ReportCard
                  title="Total Clients"
                  value={overview.kpis.totalClients}
                  icon={Users}
                  accentColor="purple"
                />
                <ReportCard
                  title="Total Holdings"
                  value={overview.kpis.totalHoldings}
                  icon={Building2}
                  accentColor="sky"
                />
                <ReportCard
                  title="Active Bookings"
                  value={overview.kpis.activeBookings}
                  icon={CalendarClock}
                  accentColor="teal"
                />
                <ReportCard
                  title="Maintenance Cost (Month)"
                  value={formatCurrency(overview.kpis.maintenanceCostThisMonth)}
                  icon={Wrench}
                  accentColor="orange"
                />
              </div>
          ...
          {loading && !holdingData ? (
            ...
            <>
              <div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
                <ReportCard
                  title="Total Holdings"
                  value={holdingData.occupancy.totalHoldings}
                  icon={Building2}
                  accentColor="indigo"
                />
                <ReportCard
                  title="Available"
                  value={holdingData.occupancy.availableHoldings}
                  icon={Building2}
                  accentColor="emerald"
                />
                <ReportCard
                  title="Booked"
                  value={holdingData.occupancy.bookedHoldings}
                  icon={CalendarClock}
                  accentColor="sky"
                />
                <ReportCard
                  title="Under Maintenance"
                  value={holdingData.occupancy.underMaintenanceHoldings}
                  icon={Wrench}
                  accentColor="amber"
                />
                <ReportCard
                  title="Occupancy Rate"
                  value={`${holdingData.occupancy.occupancyRate.toFixed(1)}%`}
                  icon={Percent}
                  accentColor="purple"
                />
              </div>
              ...
              <OccupancyGauge
                data={[
                  { name: "Available", value: holdingData.occupancy.availableHoldings, fill: "hsl(160, 60%, 50%)" },
                  { name: "Booked", value: holdingData.occupancy.bookedHoldings, fill: "hsl(240, 70%, 60%)" },
                  { name: "Maintenance", value: holdingData.occupancy.underMaintenanceHoldings, fill: "hsl(35, 90%, 55%)" },
                  { name: "Inactive", value: holdingData.occupancy.inactiveHoldings, fill: "hsl(0, 0%, 55%)" },
                ]}
              />
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <Card className="border-border/50">
                  <CardHeader>
                    ...
                    <CardDescription>Last 20 maintenance records</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {maintenanceData.recentRecords.length === 0 ? (
                            <tr>
